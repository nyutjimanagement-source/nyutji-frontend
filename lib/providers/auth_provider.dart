...
class AuthProvider with ChangeNotifier {
  ...
  Future<String?> register(Map<String, dynamic> regData) async {
    ...
    try {
      ...
      String lowErr = rawError.toLowerCase();
      if (lowErr.contains('email') && (lowErr.contains('already') || lowErr.contains('unique'))) {
        return 'Alamat Email sudah terdaftar! Gunakan email lain.';
      }
      if ((lowErr.contains('phone') || lowErr.contains('nomor')) && (lowErr.contains('already') || lowErr.contains('unique'))) {
        return 'Nomor Handphone sudah digunakan! Gunakan nomor lain.';
      }
      if (lowErr.contains('password') && (lowErr.contains('short') || lowErr.contains('length'))) {
