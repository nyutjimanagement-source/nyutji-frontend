...
class _CustomerProfileScreenState extends ConsumerState<CustomerProfileScreen> {
          ...
          children: [
            ...
            _buildSettingsGroup([
              ...
              _settingRow(LucideIcons.bell, currentT['notif'], onTap: () {
                NyutjiNotif.showInfo(
                    context, "Layanan Notifikasi akan Segera Hadir");
              }),
            ...
            _buildSettingsGroup([
              _settingRow(LucideIcons.logOut, currentT['logout'],
                  isDanger: true, onTap: () async {
  ...
  Widget _buildPremiumHeader(Map<String, dynamic> currentT, AuthProvider auth) {
    ...
    final String finalUrl = ApiConstants.profilePhotoUrl(photoUrl);
                ...
                decoration: BoxDecoration(
                  ...
                  image: kIsWeb
                      ? (auth.temporaryWebBytes != null
                          ...
                          : (finalUrl.isNotEmpty)
                              ? DecorationImage(
                                  image: CachedNetworkImageProvider(finalUrl),
                                  fit: BoxFit.cover)
                              : null)
                      : (localPhoto != null
                          ? DecorationImage(
                ...
                children: [
                  auth.isLoading
                      ? const ShimmerLoading(
                          height: 24, width: 150, borderRadius: 4)
                      : Text(
                          auth.user?['name'] ?? "Pelanggan",
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: NyutjiTheme.h2(Colors.white)
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: const Color(0xFFDAC66F),
                        borderRadius: BorderRadius.circular(4)),
                    child: Text(currentT['tier'],
  ...
  Widget _buildExpandableAddressRow(
      Map<String, dynamic> currentT, AuthProvider auth) {
              ...
              ? Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          const Icon(LucideIcons.home,
                              size: 14, color: Color(0xFF403600)),
                      ...
                      auth.isLoading
                          ? const ShimmerLoading(
                              height: 18, width: 250, borderRadius: 4)
                          : Text(mainAddress,
                              style: NyutjiTheme.body(NyutjiTheme.darkText)
                                  .copyWith(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 13)),
                      if (addressDetail.isNotEmpty) ...[
                        ...
                        auth.isLoading
                            ? const ShimmerLoading(
                                height: 14, width: 200, borderRadius: 4)
                            : Text(addressDetail,
                                style: NyutjiTheme.detail(Colors.grey[600]!)),
                      ],
                      const SizedBox(height: 16),
                      GestureDetector(
                        onTap: () {
                        ...
                        child: Container(
                          ...
                          child: Text("+ Tambah Alamat",
                              style: NyutjiTheme.body(Colors.grey[600]!)
                                  .copyWith(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 13)),
                        ),
  ...
  Widget _buildExpandableSettingsRow(
      Map<String, dynamic> currentT, AuthProvider auth) {
    final user = auth.user;
    final name = user?['name']?.toString() ?? '-';
    final phone =
        user?['phone']?.toString() ?? user?['no_hp']?.toString() ?? '-';
    final email = user?['email']?.toString() ?? '-';
    final tier = currentT['tier'] ?? 'VIP Member';
    final hasName = user?['name'] != null &&
        user!['name'].toString().isNotEmpty &&
        user['name'].toString() != '-';
    final hasPhone = (user?['phone'] != null &&
              ...
              ? Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildReadOnlyTextField("Membership", tier,
                          suffix: Tooltip(
                            message:
                                "Kategori Membership terkait jumlah walletNyutji tersimpan dan Layanan Prima Nyutji",
                            ...
                            textStyle: NyutjiTheme.detail(Colors.white)
                                .copyWith(
