...
class _MitraOrderScreenState extends State<MitraOrderScreen> {
    ...
    return Scaffold(
      ...
      body: Consumer<OrderProvider>(
        builder: (context, orderProv, _) {
          ...
          // EFEK SLIDE ANTAR FILTER: Menggunakan AnimatedSwitcher
            ...
            transitionBuilder: (Widget child, Animation<double> animation) {
              return SlideTransition(
                position: Tween<Offset>(
                  begin: const Offset(0.3, 0.0), // Bergeser sedikit dari kanan
                  end: Offset.zero,
                ).animate(animation),
  ...
  Widget _buildListContent(List<dynamic> filtered, bool isLoading, bool hasNewOrders) {
    ...
    // LOGIKA CHUNKING: 5 Record per Halaman
    ...
    return Column(
      key: ValueKey('list_$currentFilter'), // Penting untuk animasi switcher
      children: [
        Expanded(
          child: RefreshIndicator(
            ...
            child: PageView.builder(
              ...
              itemBuilder: (context, pIdx) {
                final pageItems = pages[pIdx];
                return ListView.builder(
                  padding: const EdgeInsets.fromLTRB(16, 12, 16, 20),
                  itemCount: pageItems.length,
                  physics: const BouncingScrollPhysics(),
                  itemBuilder: (context, iIdx) => _buildPremiumOrderCard(pageItems[iIdx], hasNewOrders),
                );
              },
            ),
          ),
        ...
        if (pages.length > 1)
              ...
              children: List.generate(pages.length, (index) {
                ...
                return AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    color: isAct ? primaryTeal : Colors.grey[300],
          ...
          const SizedBox(height: 80), // Padding bawah jika cuma 1 halaman
      ],
    );
  }
    ...
    return GestureDetector(
      onTap: () {
        if (currentFilter != label) {
          setState(() {
            currentFilter = label;
            _currentPage = 0; // Reset ke halaman pertama saat ganti filter
          });
          // Reset PageView position
